...
from ..models import ForecastPoint, Metric, MissingReason
...
class ForecastSourceUnavailableError(RuntimeError):
    """두 시간별 예보 소스 모두 응답하지 않아 수집 상태를 남겨야 하는 경우."""
...
class OpenMeteoProvider:
    ...
    async def fetch_spot(self, spot: Spot) -> ProviderReading | None:
        ...
        return ProviderReading(
            ...
            metric_missing_reasons={
                Metric.TIDE_LEVEL: MissingReason.SOURCE_NOT_SUPPORTED,
            },
        )
    ...
    async def fetch_forecast_series(self, spot: Spot) -> list[ForecastPoint]:
        """'가장 안전한 시간' 산정용 시간별 예보. 두 소스를 병렬 조회한다."""
        ...
        if marine is None and forecast is None:
            raise ForecastSourceUnavailableError("hourly marine and wind sources unavailable")
